...
function deleteNoMarkupFormatHTML(content) {
  const token = "`";
  const options = {
    whiteList: [],
    stripIgnoreTag: true,
  ...
  for (let i = 1; i < contentLength; i += 2) {
    finalContent += (0, import_xss.default)(splitContent[i - 1], options);
    finalContent += token + splitContent[i] + token;
  }
  if (contentLength % 2 === 1) {
    finalContent += (0, import_xss.default)(splitContent[contentLength - 1], options);
  }
  return finalContent;
}
function addS(text, num) {
  return num === 1 ? text : `${text}s`;
}
var showCollapseOrExpandMessage = (isCollapsed) => isCollapsed ? SHOW_MORE_TEXT : SHOW_LESS_TEXT;
var formatCollapsingText = (text, shouldCollapse, isCollapsed, minLength) => {
...
//# sourceMappingURL=/build/_shared/chunk-HZDGKNSJ.js.map
