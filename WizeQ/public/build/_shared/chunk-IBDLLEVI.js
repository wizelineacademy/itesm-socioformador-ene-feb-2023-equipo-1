...
var propTypes = {
  href: import_prop_types2.default.string.isRequired,
  children: import_prop_types2.default.node.isRequired
};
var buildLinkAttributes = (href) => {
  let target = "_blank";
...
// app/components/SearchedLinkRenderer/SearchedLinkRenderer.jsx
var propTypes2 = {
  href: import_prop_types3.default.string.isRequired,
...
function SearchedLinkRenderer({ href, children }) {
  ...
  return /* @__PURE__ */ import_react4.default.createElement(MarkdownLinkRenderer_default, {
    href: cleanedUrl
  }, children);
}
...
// app/utils/markdownFormatQuestions.js
var addTagsToText = (srcText) => srcText.replace(/(#[a-z\d]+)/ig, (x) => ` [${x}](/?tag=${x.slice(1)})`);
var removeQuestionLinksInsideCode = (srcText) => srcText.replace(/`.*?`/g, (x) => x.replace(/\[(Q\d+)\]\(\/\?questionId=\d+\)/g, "$1"));
var addQuestionsToText = (srcText) => removeQuestionLinksInsideCode(srcText.replace(/\b(?!`)\b(Q[0-9]+)\b(?!`)/g, (x) => `[${x}](/?questionId=${x.slice(1)})`));
var highlightText = (srcText, searchTerm) => {
  const searchRegExp = new RegExp(searchTerm.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "gi");
  let formattedText = srcText.replace(searchRegExp, (text4) => HIGHLIGHT_START + text4 + HIGHLIGHT_END);
  formattedText = (0, import_server.renderToStaticMarkup)(/* @__PURE__ */ import_react5.default.createElement(ReactMarkdown, {
...
var markdownFormat = (srcText, searchTerm) => {
...
var markdownFormatQuestion = (srcText, searchTerm) => {
  const formattedText = addQuestionsToText(srcText);
  return markdownFormat(formattedText, searchTerm);
};
var markdownFormatQuestions_default = markdownFormatQuestion;
 ...
 * LICENSE file in the root directory of this source tree.
 */
//# sourceMappingURL=/build/_shared/chunk-IBDLLEVI.js.map
